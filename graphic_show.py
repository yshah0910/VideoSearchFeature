...
def search_videos():
    global found_videos  # Access the global variable found_videos
    keywords = entry.get().split()  # Split entered keywords by spaces
    placeholders = ' OR '.join(['description LIKE ?' for _ in range(len(keywords))])  # Create placeholders for each keyword
...
def update_result_list(videos):
    ...
    for video in videos:
        result_listbox.insert(tk.END, video[1])  # Display only the video name (assuming it's in the second column)
...
def play_video():
    ...
    if selected_index:
        ...
        video_path = selected_video[3]  # Assuming path is in the fourth column (index 3)
